...
Private DialogBox As DialogSheet
...
Public Property Get GetUserName() As String
  Attribute GetUserName.VB_Description = "r300 Получить имя текущей учётной записи"
  GetUserName = Environ("UserName")
End Property

Private Sub Auto_Open() ' Подсчёт CRC_HOST = SUM( 2 ^ (item - 1) )
  ...
  Next modulo: If IsEmpty(item) Then GetForm_DialogElements dtRange, Paths
  'If IsEmpty(item) Then item = MsgBox(Paths(1) & Paths(2), _
    vbYesNo + vbExclamation, Paths(3)): If item = vbYes Then Stop
  '-> NEXT
...
Static Sub DialogButtons_Click()
  ...
  If DialogBox Is Nothing Then Exit Sub ' HotFix!
  With DialogBox
...
Private Sub GetForm_DialogElements(ByVal DType As DialogType, _
  ...
  Application.DisplayAlerts = False
    ...
    Wend: Set DialogBox = DialogSheets.Add
    With DialogBox
...
Private Sub GetWorkbooks(ByVal strDir As String) ' Все статистики
  Attribute GetWorkbooks.VB_Description = "r310 Запись найденных баз/статистик в коллекцию"
    ...
    If (GetAttr(strName) And vbDirectory) = vbDirectory Then
      With ThisWorkbook
        If DirName.Count = 0 And Right(.Name, 2) = "sm" Then _
          WriteLog Left(strName, InStr(12, strName, "\")) & "YCHET\Архив\", _
      ...
      Do While strName <> vbNullString ' Выполнять ПОКА
        ...
        And Not strName Like "*.lnk" And (IIf(strDir Like "*\Баз*", _
  ...
  ErrDir:
  If Not strName Like "*\*.*" And Err.Number = 53 Then Err.Number = 75
  ...
  End Select: MsgBox strName & vbCr & GetMainPath & strDir _
...
Function ChoiceCategory(ByVal row As Integer) As Byte
  Attribute ChoiceCategory.VB_Description = "r270 Матрица"
...
Private Property Get GetMainPath(Optional ByVal DiskOnly As Boolean) As String
  Attribute GetMainPath.VB_Description = "r302 Определить директорию/диск для поиска статистики"
